...
namespace Game
{
	public class WordNexus_GridBoardWorker : WordNexus_Worker
	{
		#region Classes

		public class WordNexus_Board
		{
			public int numRows;
			public int numCols;
			public List<WordNexus_BoardWord> boardWords;
			public char[][] letters;    // Only used when displaying the board in the LevelBuilderWindow
		}

		public class WordNexus_BoardWord
		{
			public string word;
		...
		private class WordNexus_Grid
		{
			public List<List<WordNexus_GridCell>> gridCells;
			public int numRows;
			public int numCols;
			public int rowOrigin;
			public int colOrigin;
			public List<WordNexus_GridCell> placedLetters;
			public int numPlacedWords;
			public bool hasBonusWord;
			public WordNexus_GridWord bonusWord;
		}

		private class WordNexus_GridCell
		{
		...
		private class WordNexus_CrossLetter
		{
			public char letter = '\0';
			public int offset = 0;
		}

		private class WordNexus_GridWord
		{
			public string word;
			public List<WordNexus_GridWordLetter> gridWordLetters;
		...
		private class WordNexus_GridWordLetter
		{
			public WordNexus_GridWord gridWord;
			public char letter;
			public int index;
		}
		...
		// Out Values
		public WordNexus_Board BestBoard { get; set; }
		...
		{
			WordNexus_Grid grid = CreateGrid(BonusWord);
			List<WordNexus_GridWord> gridWords = CreateGridWords(Words);
			Dictionary<char, List<WordNexus_GridWordLetter>> letterMap = CreateLetterMap(gridWords);

			// Get the list of starting GridCells
			List<WordNexus_GridCell> startingGridCells = GetStartingGridCells(grid);

			// Create an empty list of grid boards, we will fill this with all possible boards
			List<WordNexus_Board> gridBoards = new List<WordNexus_Board>();
			...
			else
			{
				...
				{
					WordNexus_GridCell startingGridCell = startingGridCells[i];

					for (int j = 0; j < gridWords.Count; j++)
					{
						WordNexus_GridWord gridWord = gridWords[j];
		...
		/// </summary>
		public static void SetBoardLettersArray(WordNexus_Board board)
		{
			// Initalize the board
			...
			{
				WordNexus_BoardWord boardWord = board.boardWords[i];
		...
		/// </summary>
		private List<WordNexus_GridWord> CreateGridWords(List<string> words)
		{
			List<WordNexus_GridWord> gridWords = new List<WordNexus_GridWord>();
		...
		/// </summary>
		private Dictionary<char, List<WordNexus_GridWordLetter>> CreateLetterMap(List<WordNexus_GridWord> gridWords)
		{
			Dictionary<char, List<WordNexus_GridWordLetter>> letterMap = new Dictionary<char, List<WordNexus_GridWordLetter>>();

			for (int i = 0; i < gridWords.Count; i++)
			{
				WordNexus_GridWord gridWord = gridWords[i];

				for (int j = 0; j < gridWord.gridWordLetters.Count; j++)
				{
					WordNexus_GridWordLetter gridWordLetter = gridWord.gridWordLetters[j];
					char letter = gridWordLetter.letter;

					if (!letterMap.ContainsKey(letter))
					{
						// Add a new letter list to the map
						letterMap.Add(letter, new List<WordNexus_GridWordLetter>());
					}
		...
		/// </summary>
		private WordNexus_Grid CreateGrid(string bonusWord)
		{
			WordNexus_Grid grid = new WordNexus_Grid
			{
				...
				numCols = 1,
				numRows = 1,
				gridCells = new List<List<WordNexus_GridCell>>() { new List<WordNexus_GridCell>() { CreateGridCell(0, 0) } },
				placedLetters = new List<WordNexus_GridCell>()
			};

			if (!string.IsNullOrEmpty(bonusWord))
			{
				WordNexus_GridWord bonusGridWord = CreateGridWord(bonusWord);
		...
		/// </summary>
		private WordNexus_GridWord CreateGridWord(string word)
		{
			WordNexus_GridWord gridWord = new WordNexus_GridWord
			{
				word = word,
				isPlaced = false
			};
		...
		/// </summary>
		private List<WordNexus_GridWordLetter> CreateGridWordLetters(WordNexus_GridWord gridWord)
		{
			List<WordNexus_GridWordLetter> gridWordLetters = new List<WordNexus_GridWordLetter>();

			for (int i = 0; i < gridWord.word.Length; i++)
			{
				WordNexus_GridWordLetter gridWordLetter = CreateGridWordLetter(gridWord, i);
		...
		/// </summary>
		private WordNexus_GridWordLetter CreateGridWordLetter(WordNexus_GridWord gridWord, int index)
		{
			WordNexus_GridWordLetter gridWordLetter = new WordNexus_GridWordLetter
			{
				gridWord = gridWord,
		...
		/// </summary>
		private WordNexus_GridCell CreateGridCell(int rowOriginOffset, int colOriginOffset, bool canPlaceLetter = true, char letter = (char)0)
		{
			WordNexus_GridCell gridCell = new WordNexus_GridCell
			{
		...
		/// </summary>
		private List<WordNexus_GridCell> GetStartingGridCells(WordNexus_Grid grid)
		{
			List<WordNexus_GridCell> startingGridCells = new List<WordNexus_GridCell>();

			for (int i = 0; i < grid.gridCells.Count; i++)
			{
				for (int j = 0; j < grid.gridCells[i].Count; j++)
				{
					WordNexus_GridCell gridCell = grid.gridCells[i][j];
		...
		/// </summary>
		private void BeginCreateBoards(WordNexus_Grid grid,
									   List<WordNexus_GridWord> gridWords,
									   Dictionary<char, List<WordNexus_GridWordLetter>> letterMap,
									   List<WordNexus_Board> gridBoards,
									   WordNexus_GridWord startingGridWord,
									   int rowIndex,
									   int colIndex,
		...
		/// </summary>
		private void PlaceWordAndCreateBoards(WordNexus_Grid grid,
											 List<WordNexus_GridWord> gridWords,
											 Dictionary<char, List<WordNexus_GridWordLetter>> letterMap,
											 List<WordNexus_Board> gridBoards,
											 WordNexus_GridWord gridWord,
											 int rowIndex,
											 int colIndex,
											 Direction direction)
		{
		...
		/// </summary>
		private int PlaceWord(WordNexus_Grid grid, WordNexus_GridWord gridWord, int rowIndex, int colIndex, Direction direction)
		{
		...
		/// </summary>
		private bool PlaceLetter(WordNexus_Grid grid, char letter, int rowIndex, int colIndex)
		{
			...
			WordNexus_GridCell gridCell = grid.gridCells[rowIndex][colIndex];
		...
		/// </summary>
		private void ClearPlacedLetters(WordNexus_Grid grid, int numToClear)
		{
			for (int i = 0; i < numToClear; i++)
			{
				int index = grid.placedLetters.Count - 1;

				WordNexus_GridCell gridCell = grid.placedLetters[index];
		...
		/// </summary>
		private void AddRows(WordNexus_Grid grid, int numRows, bool atBeginning)
		{
		...
		/// </summary>
		private void AddCols(WordNexus_Grid grid, int numCols, bool atBeginning)
		{
		...
		/// </summary>
		private void AddRow(WordNexus_Grid grid, bool insertAtBeginning)
		{
			...
			List<WordNexus_GridCell> rowGridCells = new List<WordNexus_GridCell>();
		...
		/// </summary>
		private void AddCol(WordNexus_Grid grid, bool insertAtBeginning)
		{
			...
			{
				WordNexus_GridCell gridCell = CreateGridCell(i - grid.rowOrigin, colOriginOffset);
		...
		/// </summary>
		private void CreateBoards(WordNexus_Grid grid,
								  List<WordNexus_GridWord> gridWords,
								  Dictionary<char, List<WordNexus_GridWordLetter>> letterMap,
								  List<WordNexus_Board> gridBoards)
		{
			...
			for (int i = 0; i < grid.placedLetters.Count; i++)
			{
				...
				WordNexus_GridCell gridCell = grid.placedLetters[i];
				int rowIndex = GetRowIndex(grid, gridCell);
				int colIndex = GetColIndex(grid, gridCell);

				List<WordNexus_GridWordLetter> gridWordLetters = GetPlaceableGridWordLetters(letterMap, gridCell.letter);
		...
		/// </summary>
		private void TryPlaceWords(WordNexus_Grid grid,
								   List<WordNexus_GridWord> gridWords,
								   Dictionary<char, List<WordNexus_GridWordLetter>> letterMap,
								   List<WordNexus_Board> gridBoards,
								   Direction direction,
								   List<WordNexus_GridWordLetter> gridWordLetters,
								   WordNexus_GridCell gridCell,
								   int crossWorStartOffset,
								   int crossWordEndOffset,
								   List<int> crossLetterOffsets)
		{
			for (int i = 0; i < gridWordLetters.Count; i++)
			{
				...
				WordNexus_GridWordLetter gridWordLetter = gridWordLetters[randIndex];
		...
		/// </summary>
		private bool CanPlaceWord(WordNexus_Grid grid, WordNexus_GridWordLetter gridWordLetter, Direction direction, int rowIndex, int colIndex, int crossWorStartOffset, int crossWordEndOffset, List<int> crossLetterOffsets)
		{
			int gridIndex = (direction == Direction.Vertical ? rowIndex : colIndex);

			WordNexus_GridWord gridWord = gridWordLetter.gridWord;
		...
		/// </summary>
		private bool CanPlaceAnyWordVertically(WordNexus_Grid grid, int rowIndex, int colIndex, out int crossWordStartOffset, out int crossWordEndOffset, out List<int> crossLetterOffsets)
		{
		...
		/// </summary>
		private bool CanPlaceAnyWordHorizontally(WordNexus_Grid grid, int rowIndex, int colIndex, out int crossWordStartOffset, out int crossWordEndOffset, out List<int> crossLetterOffsets)
		{
			crossWordStartOffset = -1;
			crossWordEndOffset = -1;
			crossLetterOffsets = new List<int>();
		...
		/// </summary>
		private List<WordNexus_GridWordLetter> GetPlaceableGridWordLetters(Dictionary<char, List<WordNexus_GridWordLetter>> letterMap, char letter)
		{
			List<WordNexus_GridWordLetter> gridWordLetters = letterMap.ContainsKey(letter) ? letterMap[letter] : null;
			List<WordNexus_GridWordLetter> placeableWords = new List<WordNexus_GridWordLetter>();

			for (int i = 0; i < gridWordLetters.Count; i++)
			{
				WordNexus_GridWordLetter gridWordLetter = gridWordLetters[i];
		...
		/// </summary>
		private int GetRowIndex(WordNexus_Grid grid, WordNexus_GridCell gridCell)
		{
		...
		/// </summary>
		private int GetColIndex(WordNexus_Grid grid, WordNexus_GridCell gridCell)
		{
		...
		/// </summary>
		private bool IsPartOfVerticalWord(WordNexus_Grid grid, int rowIndex, int colIndex)
		{
		...
		/// Checks if the cell at the given index is part of a vertical word
		/// </summary>
		private bool IsPartOfHorizontalWord(WordNexus_Grid grid, int rowIndex, int colIndex)
		{
		...
		/// </summary>
		private bool HasLetter(WordNexus_Grid grid, int rowIndex, int colIndex)
		{
		...
		/// Checks if the cell at the given row/col index has a letter placed on it
		/// </summary>
		private bool CanPlace(WordNexus_Grid grid, int rowIndex, int colIndex)
		{
		...
		/// </summary>
		private WordNexus_Board CreateBoard(WordNexus_Grid grid, List<WordNexus_GridWord> gridWords)
		{
			WordNexus_Board board = new WordNexus_Board();

			board.boardWords = new List<WordNexus_BoardWord>();
			...
			{
				WordNexus_GridWord gridWord = (i == -1 ? grid.bonusWord : gridWords[i]);
				WordNexus_BoardWord boardWord = new WordNexus_BoardWord();
		...
		/// </summary>
		private void NormalizeBoard(WordNexus_Board board)
		{
		...
		/// </summary>
		private WordNexus_Board GetBestBoard(List<WordNexus_Board> boards)
		{
			if (boards.Count == 0)
			{
				return null;
			...
			List<WordNexus_Board> bestBoards = new List<WordNexus_Board>() { boards[0] };
			float bestValue = CalculateValue(boards[0]);

			for (int i = 0; i < boards.Count; i++)
			{
				WordNexus_Board board = boards[i];
				...
				{
					bestBoards = new List<WordNexus_Board>() { board };
		...
		/// </summary>
		private float CalculateValue(WordNexus_Board board)
		{
		...
		/// </summary>
		private void PrintBoard(WordNexus_Board board)
		{
			char[][] letters = new char[board.numRows][];
		...
		/// Prints the grid cells
		/// </summary>
		private void PrintBoardInfo(WordNexus_Board board)
		{
			string print = "";
		...
		/// Prints the grid cells
		/// </summary>
		private void DebugPrintGridCells(WordNexus_Grid grid)
		{
			string print = "";
		...
		/// Prints the grid cells
		/// </summary>
		private void DebugPrintRowOffsets(WordNexus_Grid grid)
		{
			string print = "";
		...
		/// Prints the grid cells
		/// </summary>
		private void DebugPrintColOffsets(WordNexus_Grid grid)
		{
			string print = "";
