...
namespace Game
{
	public class WordNexus_SoundManager : SingletonComponent<WordNexus_SoundManager>, WordNexus_ISaveable
	{
		#region Classes

		[System.Serializable]
		private class WordNexus_SoundInfo
		{
			public string id = "";
		...
		private class WordNexus_PlayingSound
		{
			public WordNexus_SoundInfo soundInfo = null;
		...
		[SerializeField] private List<WordNexus_SoundInfo> soundInfos = null;

		#endregion

		#region Member Variables

		private List<WordNexus_PlayingSound> playingAudioSources;
		private List<WordNexus_PlayingSound> loopingAudioSources;
		...
		protected override void Awake()
		{
			...
			playingAudioSources = new List<WordNexus_PlayingSound>();
			loopingAudioSources = new List<WordNexus_PlayingSound>();
		...
		private void Start()
		{
			...
			{
				WordNexus_SoundInfo soundInfo = soundInfos[i];
		...
		{
			WordNexus_SoundInfo soundInfo = GetSoundInfo(id);
			...
			WordNexus_PlayingSound playingSound = new WordNexus_PlayingSound();
		...
		/// </summary>
		private void StopAllSounds(string id, List<WordNexus_PlayingSound> playingSounds)
		{
			for (int i = 0; i < playingSounds.Count; i++)
			{
				WordNexus_PlayingSound playingSound = playingSounds[i];
		...
		/// </summary>
		private void StopAllSounds(SoundType type, List<WordNexus_PlayingSound> playingSounds)
		{
			for (int i = 0; i < playingSounds.Count; i++)
			{
				WordNexus_PlayingSound playingSound = playingSounds[i];
		...
		private WordNexus_SoundInfo GetSoundInfo(string id)
		{
			for (int i = 0; i < soundInfos.Count; i++)
			{
