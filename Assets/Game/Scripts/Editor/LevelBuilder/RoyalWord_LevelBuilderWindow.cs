...
namespace Game
{
	public class RoyalWord_LevelBuilderWindow : EditorWindow
	{
		#region Classes

		private class RoyalWord_PreviewLetter
		{
			public char letter;
			public bool isNew;
		...
		private Dictionary<string, List<RoyalWord_RankedWord>> wordDictionary;
		private Dictionary<int, List<RoyalWord_RankedWord>> wordsByLength;
		...
		private List<RoyalWord_PreviewLetter> previewLetters;         // Sorted letters that contains both choosenSortedLetters and the selectedWord
		private List<RoyalWord_RankedWord> possibleWords;
		private RoyalWord_GridBoardWorker.RoyalWord_Board board;
		private RoyalWord_GridBoardWorker gridBoardWorker;
		private RoyalWord_BatchLevelCreationWorker batchLevelCreationWorker;
		...
		private Dictionary<string, List<RoyalWord_RankedWord>> WordDictionary { get { return (wordDictionary == null) ? wordDictionary = new Dictionary<string, List<RoyalWord_RankedWord>>() : wordDictionary; } }
		private Dictionary<int, List<RoyalWord_RankedWord>> WordsByLength { get { return (wordsByLength == null) ? wordsByLength = new Dictionary<int, List<RoyalWord_RankedWord>>() : wordsByLength; } }
		private List<RoyalWord_PreviewLetter> PreviewLetters { get { return (previewLetters == null) ? previewLetters = new List<RoyalWord_PreviewLetter>() : previewLetters; } }
		...
		private List<RoyalWord_RankedWord> PossibleWords { get { return (possibleWords == null) ? possibleWords = new List<RoyalWord_RankedWord>() : possibleWords; } set { possibleWords = value; } }
		...
		/// </summary>
		private Texture2D LineTexture { get { return (lineTexture != null) ? lineTexture : (lineTexture = RoyalWord_Utilities.CreateTexture(1, 1, GUI.skin.label.normal.textColor)); } }
		...
		private void DrawLevelWords(float width, float height)
		{
			...
			while (lettersPerRow > 0 && letterIndex < PreviewLetters.Count)
			{
				...
				{
					RoyalWord_PreviewLetter previewLetter = PreviewLetters[letterIndex];
		...
		private void DrawExportSettings()
		{
			...
			string folderPath = RoyalWord_LevelBuilderUtilities.GetOutputFolderPath(outputFolder);
		...
		private void ProcessWordFile()
		{
			...
			for (int i = 0; i < words.Length; i++)
			{
				...
				string sortedWord = RoyalWord_LevelBuilderUtilities.SortAlphabetically(wordStr);
				RoyalWord_RankedWord word = new RoyalWord_RankedWord(wordStr, i);

				if (!WordDictionary.ContainsKey(sortedWord))
				{
					WordDictionary.Add(sortedWord, new List<RoyalWord_RankedWord>());
				}
				...
				{
					WordsByLength.Add(wordStr.Length, new List<RoyalWord_RankedWord>());
				}
		...
		{
			choosenSortedLetters = RoyalWord_LevelBuilderUtilities.GetLetters(ChoosenWords);
		}
		...
		private void UpdatePreviewLetters()
		{
			...
			for (int i = 0; i < customWord.Length; i++)
			{
				...
				RoyalWord_PreviewLetter previewLetter = new RoyalWord_PreviewLetter();
			...
			PreviewLetters.Sort((RoyalWord_PreviewLetter x, RoyalWord_PreviewLetter y) =>
				{
		...
		private void UpdatePossibleWords()
		{
			...
			{
				PossibleWords = RoyalWord_LevelBuilderUtilities.GetWordsThatFit(choosenSortedLetters, letterCount, WordDictionary, WordsByLength);
			}
		...
		private void UpdateBoard()
		{
			...
			{
				// Create a new worker
				gridBoardWorker = new RoyalWord_GridBoardWorker();
		...
		{
			batchLevelCreationWorker = new RoyalWord_BatchLevelCreationWorker();
			...
			batchLevelCreationWorker.LoadFromFolderPath = RoyalWord_LevelBuilderUtilities.GetFolderPath(loadWordsFromFolder);
			...
			batchLevelCreationWorker.OutputFolderPath = RoyalWord_LevelBuilderUtilities.GetOutputFolderPath(outputFolder);
		...
		private void ExportLevel(string folderPath)
		{
			...
			bool fileCreated = RoyalWord_LevelBuilderUtilities.ExportLevelFile(choosenSortedLetters, boardType, board, ChoosenWords, ChoosenWordsStar, false, 0, folderPath, filename, false);
			...
			if (!fileCreated)
			{
				if (EditorUtility.DisplayDialog("File Already Exists", "The level file already exists in the given folder, would you like to overwrite it?", "Yes", "No"))
				{
					...
					fileCreated = RoyalWord_LevelBuilderUtilities.ExportLevelFile(choosenSortedLetters, boardType, board, ChoosenWords, ChoosenWordsStar, false, 0, folderPath, filename, true);
		...
		private void ImportLevelFile(TextAsset levelFile)
		{
			...
			BoardType newBoardType;
			RoyalWord_GridBoardWorker.RoyalWord_Board newBoard;

			bool loaded = RoyalWord_LevelBuilderUtilities.LoadLevelFile(levelFile.text, out letters, out wordsInLevel, out wordIsStarred, out newBoardType, out newBoard);
		...
		private void SortPossibleWords()
		{
			...
			{
				case SortType.Rank:
					PossibleWords.Sort((RoyalWord_RankedWord word1, RoyalWord_RankedWord word2) =>
					{
