...
namespace Game
{
	public class RoyalWord_RecyclableListHandler<T>
	{
		#region Classes
		...
		private RoyalWord_RecyclableListItem<T> listItemPrefab;
		...
		public System.Action<RoyalWord_RecyclableListItem<T>> OnListItemCreated { get; set; }
		...
		public RoyalWord_RecyclableListHandler(List<T> dataObjects, RoyalWord_RecyclableListItem<T> listItemPrefab, RectTransform listContainer, ScrollRect listScrollRect)
		{
		...
		public void Refresh()
		{
			for (int i = topItemIndex; i <= bottomItemIndex; i++)
			{
				...
				{
					RoyalWord_RecyclableListItem<T> listItem = placeholder.GetChild(0).GetComponent<RoyalWord_RecyclableListItem<T>>();
		...
		private void AddListItem(int index, RectTransform placeholder, bool addingOnTop)
		{
			...
			RoyalWord_RecyclableListItem<T> listItem = listItemPool.GetObject<RoyalWord_RecyclableListItem<T>>(placeholder, out itemInstantiated);
