...
namespace Game
{
	public class WordNexus_ScreenManager : SingletonComponent<WordNexus_ScreenManager>
	{
		...
		[SerializeField] private List<WordNexus_Screen> screens = null;
		...
		private WordNexus_Screen currentScreen;
		...
		private void Start()
		{
			...
			{
				WordNexus_Screen screen = screens[i];
		...
		private void Update()
		{
			if (Input.GetKeyDown(KeyCode.Escape))
			{
				...
				if (!WordNexus_Pop_upManager.Instance.CloseActivePopup())
				{
		...
		private void Show(string screenId, bool back, bool immediate)
		{
			...
			WordNexus_Screen screen = GetScreenById(screenId);
		...
		private WordNexus_Screen GetScreenById(string id)
		{
			for (int i = 0; i < screens.Count; i++)
			{
				if (id == screens[i].Id)
