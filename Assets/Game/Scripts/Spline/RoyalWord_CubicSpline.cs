...
namespace Game
{
	public class RoyalWord_CubicSpline
	{
		#region Fields
		...
		public RoyalWord_CubicSpline(float[] x, float[] y, float startSlope = float.NaN, float endSlope = float.NaN, bool debug = false)
		{
		...
		public void Fit(float[] x, float[] y, float startSlope = float.NaN, float endSlope = float.NaN, bool debug = false)
		{
			...
			RoyalWord_TriDiagonalMatrix m = new RoyalWord_TriDiagonalMatrix(n);
		...
		{
			RoyalWord_CubicSpline spline = new RoyalWord_CubicSpline();
			...
			float firstDx = Single.NaN, float firstDy = Single.NaN, float lastDx = Single.NaN, float lastDy = Single.NaN)
		{
			...
			RoyalWord_CubicSpline xSpline = new RoyalWord_CubicSpline();
			xs = xSpline.FitAndEval(dists, x, times, firstDx / dt, lastDx / dt);

			RoyalWord_CubicSpline ySpline = new RoyalWord_CubicSpline();
