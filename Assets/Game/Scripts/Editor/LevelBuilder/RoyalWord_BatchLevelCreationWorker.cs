...
namespace Game
{
	public class RoyalWord_BatchLevelCreationWorker : RoyalWord_Worker
	{
		...
		// In Properties
		public Dictionary<string, List<RoyalWord_RankedWord>> WordDictionary { get; set; }
		public Dictionary<int, List<RoyalWord_RankedWord>> WordsByLength { get; set; }
			...
			try
			{
				...
				for (currentBatchIndex = 0; currentBatchIndex < NumberOfLevels; currentBatchIndex++)
				{
					...
					else
					{
						...
						RoyalWord_GridBoardWorker.RoyalWord_Board board = data[2] as RoyalWord_GridBoardWorker.RoyalWord_Board;
						...
						RoyalWord_LevelBuilderUtilities.ExportLevelFile(letters, boardType, board, choosenWords, starredChoosenWords, true, levelNumber, OutputFolderPath, FilenamePrefix, OverwriteExisting);
		...
		{
			List<string> levelFiles = RoyalWord_Utilities.GetFilesRecursively(folderPath, "*.json");
			...
			for (int i = 0; i < levelFiles.Count; i++)
			{
				...
				if (RoyalWord_LevelBuilderUtilities.LoadLevelFile(contents, out letters, out wordsInLevel, out wordIsStarred, out boardType, out board))
				{
		...
		private object[] CreateLevel(List<string> choosenWords, List<string> excludedWords)
		{
			...
			string letters = RoyalWord_LevelBuilderUtilities.GetLetters(choosenWords);
		...
		private object[] FinishedCreatingLevel(string letters, List<string> choosenWords)
		{
			...
			if (CreateBoard)
			{
				RoyalWord_GridBoardWorker gridBoardWorker = new RoyalWord_GridBoardWorker();
		...
		private List<List<string>> GetPossibleWords(string letters, List<string> choosenWords, List<string> excludedWords, int minLength, int maxLength)
		{
			...
			{
				possibleWords = RoyalWord_LevelBuilderUtilities.GetWordsThatFit(letters, LetterCount, WordDictionary, WordsByLength);
			}
			...
			possibleWords.Sort((RoyalWord_RankedWord rankedWord1, RoyalWord_RankedWord rankedWord2) =>
			{
