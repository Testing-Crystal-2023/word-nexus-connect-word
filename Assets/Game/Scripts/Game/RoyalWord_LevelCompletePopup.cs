...
namespace Game
{
	public class RoyalWord_LevelCompletePopup : Game.RoyalWord_Popup
	{
		...
		public override void OnShowing(object[] inData)
		{
			...
			// Debug.LogError("show");
			RoyalWord_ActiveLevel level = (RoyalWord_ActiveLevel)inData[0];
			...
			int extraWordsCoinsAmountTo)
		{
			...
			timeEnd = RoyalWord_Utilities.SystemTimeInMilliseconds + duration;

			while (true)
			{
				float t = 1f - (float)(timeEnd - RoyalWord_Utilities.SystemTimeInMilliseconds) / duration;
				int gp = (int)Mathf.Lerp(1, awardedGamePoints, t);

				//gamePointsText.text = (currentGamePoints + gp).ToString();

				if (RoyalWord_Utilities.SystemTimeInMilliseconds >= timeEnd)
				{
					break;
				}
