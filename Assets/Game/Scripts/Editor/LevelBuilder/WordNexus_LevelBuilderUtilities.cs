...
namespace Game
{
	public class WordNexus_RankedWord
	{
		public string word;
		public int rank;

		public WordNexus_RankedWord(string word, int rank)
		{
			this.word = word;
			this.rank = rank;
		}
	...
	public static class WordNexus_LevelBuilderUtilities
	{
		...
		/// </summary>
		public static List<WordNexus_RankedWord> GetWordsThatFit(string letters, int length, Dictionary<string, List<WordNexus_RankedWord>> wordDictionary, Dictionary<int, List<WordNexus_RankedWord>> wordsByLength)
		{
			string sortedLetters = SortAlphabetically(letters);
			List<string> sortedLetterCombinations = GetLetterCombinations(sortedLetters);
			List<WordNexus_RankedWord> words = new List<WordNexus_RankedWord>();
			...
			if (extraLetters > 0)
			{
				...
				for (int i = letters.Length + 1; i <= length; i++)
				{
					...
					{
						List<WordNexus_RankedWord> wordsToTry = wordsByLength[wordLength];
		...
		public static bool ExportLevelFile(string letters,
										   BoardType boardType,
										   WordNexus_GridBoardWorker.WordNexus_Board board,
		...
		/// </summary>
		public static bool LoadLevelFile(string contents, out string letters, out List<string> wordsInLevel, out List<bool> wordIsStarred, out BoardType boardType, out WordNexus_GridBoardWorker.WordNexus_Board board)
		{
			letters = "";
			...
			try
			{
				...
				{
					board = new WordNexus_GridBoardWorker.WordNexus_Board();
					board.numRows = levelJson["rowCount"].AsInt;
					board.numCols = levelJson["colCount"].AsInt;
					board.boardWords = new List<WordNexus_GridBoardWorker.WordNexus_BoardWord>();
				}
				...
				foreach (JSONNode wordJson in wordsJson)
				{
					...
					if (boardType == BoardType.Grid)
					{
						WordNexus_GridBoardWorker.WordNexus_BoardWord boardWord = new WordNexus_GridBoardWorker.WordNexus_BoardWord();
				...
				if (board != null)
				{
					WordNexus_GridBoardWorker.SetBoardLettersArray(board);
				}
		...
		private static List<WordNexus_RankedWord> GetWordsThatFit(List<WordNexus_RankedWord> wordsToTry, Dictionary<char, int> letterDictionary, int minSharedLetters)
		{
			List<WordNexus_RankedWord> words = new List<WordNexus_RankedWord>();
												  ...
												  out string levelId)
		{
			...
			if (boardType == BoardType.Grid && board != null)
			{
				...
				{
					WordNexus_GridBoardWorker.WordNexus_BoardWord boardWord = board.boardWords[i];
			...
			levelId = GetLevelId(WordNexus_Utilities.ConvertToJsonString(json));
			...
			return WordNexus_Utilities.ConvertToJsonString(json);
		}
		...
		/// </summary>
		private static List<int> GetCoinIndicies(WordNexus_GridBoardWorker.WordNexus_Board board, WordNexus_GridBoardWorker.WordNexus_BoardWord boardWord, List<string> choosenWords, List<bool> starredChoosenWords)
		{
			...
			for (int i = 0; i < boardWord.word.Length; i++)
			{
				...
				else
				{
					...
					WordNexus_GridBoardWorker.WordNexus_BoardWord crossingBoardWord = GetCrossingWord(board, rowIndex, colIndex, !boardWord.isVertical);
		...
		/// </summary>
		private static WordNexus_GridBoardWorker.WordNexus_BoardWord GetCrossingWord(WordNexus_GridBoardWorker.WordNexus_Board board, int rowIndex, int colIndex, bool vertical)
		{
			for (int i = 0; i < board.boardWords.Count; i++)
			{
				WordNexus_GridBoardWorker.WordNexus_BoardWord boardWord = board.boardWords[i];
