...
namespace Game
{
	public class WordNexus_LetterWheel : WordNexus_UIMonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
	{
		#region Classes

		[System.Serializable]
		private class WordNexus_ScaleInfo
		{
		...
		[SerializeField] private WordNexus_UILine uiLine = null;
		[SerializeField] private WordNexus_LetterUI letterUIPrefab = null;
		...
		[SerializeField] private List<WordNexus_ScaleInfo> scaleInfos = null;

		#endregion

		#region Member Variables

		private WordNexus_ObjectPool letterUIPool;
		private List<WordNexus_LetterUI> letterUIs;
		private List<WordNexus_LetterUI> selectedLetterUIs;
		private State state;
		...
		{
			letterUIPool = new WordNexus_ObjectPool(letterUIPrefab.gameObject, 3, WordNexus_ObjectPool.CreatePoolContainer(transform));
			letterUIs = new List<WordNexus_LetterUI>();
			selectedLetterUIs = new List<WordNexus_LetterUI>();

			Canvas canvas = WordNexus_Utilities.GetCanvas(transform);
		...
		public void Setup(WordNexus_ActiveLevel level)
		{
			...
			// Get the scale info
			WordNexus_ScaleInfo scaleInfo = GetScaleInfo(letters.Length);
		...
		/// </summary>
		private WordNexus_ScaleInfo GetScaleInfo(int letterCount)
		{
			for (int i = 0; i < scaleInfos.Count; i++)
			{
				WordNexus_ScaleInfo scaleInfo = scaleInfos[i];
		...
		/// </summary>
		private void CreateLetters(string characters, Color packColor, WordNexus_ScaleInfo scaleInfo)
		{
			float letterScale = (scaleInfo != null) ? scaleInfo.letterScale : 1;

			for (int i = 0; i < characters.Length; i++)
			{
				WordNexus_LetterUI letterUI = letterUIPool.GetObject<WordNexus_LetterUI>(transform);
		...
		private void UpdateSelectedLetters(Vector2 localPosition)
		{
			...
			{
				WordNexus_LetterUI letterUI = letterUIs[i];
		...
		/// </summary>
		private void AddSelectedLetter(WordNexus_LetterUI letterUI)
		{
			...
			WordNexus_SoundManager.Instance.Play("letter-selected");
		}
		...
		private IEnumerator AnimateLettersToPositions(List<Vector2> letterPositions)
		{
			...
			for (int i = 0; i < letterUIs.Count; i++)
			{
				...
				anim = WordNexus_UIAnimation.PositionX(letterUI.RectT, position.x, animDuration);
				anim.style = WordNexus_UIAnimation.Style.EaseOut;
				anim.Play();

				anim = WordNexus_UIAnimation.PositionY(letterUI.RectT, position.y, animDuration);
				anim.style = WordNexus_UIAnimation.Style.EaseOut;
				anim.Play();
			}
