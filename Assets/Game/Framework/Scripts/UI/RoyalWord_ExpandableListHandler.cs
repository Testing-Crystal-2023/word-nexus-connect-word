...
namespace Game
{
	public class RoyalWord_ExpandableListHandler<T>
	{
		#region Classes

		private class RoyalWord_Animation
		{
		...
		private RoyalWord_ExpandableListItem<T> listItemPrefab;
		...
		public System.Action<RoyalWord_ExpandableListItem<T>> OnItemCreated { get; set; }
		...
		public RoyalWord_ExpandableListHandler(List<T> dataObjects, RoyalWord_ExpandableListItem<T> listItemPrefab, RectTransform listContainer, ScrollRect listScrollRect, float expandAnimDuration)
		{
		...
		public void CollapseListItem(int index, bool isExpandingNewItem = false)
		{
			...
			animation.OnAnimationFinished += (GameObject obj) =>
			{
				...
				{
					placeholder.GetChild(0).GetComponent<RoyalWord_ExpandableListItem<T>>().Collapsed();
				}
			};
		...
		public void Refresh()
		{
			for (int i = topItemIndex; i <= bottomItemIndex; i++)
			{
				...
				if (placeholder.childCount == 1)
				{
					RoyalWord_ExpandableListItem<T> listItem = placeholder.GetChild(0).GetComponent<RoyalWord_ExpandableListItem<T>>();
		...
		private void AddListItem(int index, RectTransform placeholder, bool addingOnTop)
		{
			...
			RoyalWord_ExpandableListItem<T> listItem = listItemPool.GetObject<RoyalWord_ExpandableListItem<T>>(placeholder, out itemInstantiated);
		...
		private void SetExpanded(RectTransform placeholder, bool isExpanded)
		{
			if (placeholder.childCount == 1)
			{
				placeholder.GetChild(0).GetComponent<RoyalWord_ExpandableListItem<T>>().IsExpanded = isExpanded;
			}
		}
