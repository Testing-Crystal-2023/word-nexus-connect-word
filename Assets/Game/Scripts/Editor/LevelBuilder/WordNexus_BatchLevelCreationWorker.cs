...
namespace Game
{
	public class WordNexus_BatchLevelCreationWorker : WordNexus_Worker
	{
		...
		// In Properties
		public Dictionary<string, List<WordNexus_RankedWord>> WordDictionary { get; set; }
		public Dictionary<int, List<WordNexus_RankedWord>> WordsByLength { get; set; }
			...
			try
			{
				...
				for (currentBatchIndex = 0; currentBatchIndex < NumberOfLevels; currentBatchIndex++)
				{
					...
					else
					{
						...
						WordNexus_GridBoardWorker.WordNexus_Board board = data[2] as WordNexus_GridBoardWorker.WordNexus_Board;
						...
						WordNexus_LevelBuilderUtilities.ExportLevelFile(letters, boardType, board, choosenWords, starredChoosenWords, true, levelNumber, OutputFolderPath, FilenamePrefix, OverwriteExisting);
		...
		{
			List<string> levelFiles = WordNexus_Utilities.GetFilesRecursively(folderPath, "*.json");
			...
			for (int i = 0; i < levelFiles.Count; i++)
			{
				...
				if (WordNexus_LevelBuilderUtilities.LoadLevelFile(contents, out letters, out wordsInLevel, out wordIsStarred, out boardType, out board))
				{
		...
		private object[] CreateLevel(List<string> choosenWords, List<string> excludedWords)
		{
			...
			string letters = WordNexus_LevelBuilderUtilities.GetLetters(choosenWords);
		...
		private object[] FinishedCreatingLevel(string letters, List<string> choosenWords)
		{
			...
			if (CreateBoard)
			{
				WordNexus_GridBoardWorker gridBoardWorker = new WordNexus_GridBoardWorker();
		...
		private List<List<string>> GetPossibleWords(string letters, List<string> choosenWords, List<string> excludedWords, int minLength, int maxLength)
		{
			...
			{
				possibleWords = WordNexus_LevelBuilderUtilities.GetWordsThatFit(letters, LetterCount, WordDictionary, WordsByLength);
			}
			...
			possibleWords.Sort((WordNexus_RankedWord rankedWord1, WordNexus_RankedWord rankedWord2) =>
			{
