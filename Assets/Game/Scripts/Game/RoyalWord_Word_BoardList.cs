...
namespace Game
{
	public class RoyalWord_Word_BoardList : RoyalWord_WordBoard
	{
		...
		[SerializeField] private RoyalWord_Word_BoardCell wordBoardCellPrefab = null;

		#endregion

		#region Member Variables

		private RoyalWord_ObjectPool wordBoardCellPool;
		private RoyalWord_ObjectPool columnContainerPool;
		private RoyalWord_ObjectPool rowContainerPool;
		private RoyalWord_ObjectPool cellContainerPool;

		private RectTransform boardContainer;
		private Dictionary<string, List<RoyalWord_Word_BoardCell>> wordBoardCells;
		...
		{
			wordBoardCells = new Dictionary<string, List<RoyalWord_Word_BoardCell>>();
			...
			Transform poolContainer = RoyalWord_ObjectPool.CreatePoolContainer(transform, "pool_container");

			// Create the ObjectPools for the word cell and containers
			wordBoardCellPool = new RoyalWord_ObjectPool(wordBoardCellPrefab.gameObject, 10, poolContainer);
			columnContainerPool = new RoyalWord_ObjectPool(columnContainerTemplate.gameObject, 10, poolContainer);
			rowContainerPool = new RoyalWord_ObjectPool(rowContainerTemplate.gameObject, 10, poolContainer);
			cellContainerPool = new RoyalWord_ObjectPool(cellContainerTemplate.gameObject, 10, poolContainer);
		...
		/// </summary>
		public override void Setup(RoyalWord_ActiveLevel level)
		{
			Clear();
		...
		protected override List<RoyalWord_Word_BoardCell> GetWordGridCells(WordData wordData)
		{
		...
		private void CreateWordList(RoyalWord_ActiveLevel level)
		{
			...
			for (int c = 0, wordIndex = 0; c < numColumns; c++)
			{
				...
				for (int r = 0; r < numWordsInColumn && wordIndex < level.levelData.Words.Count; r++, wordIndex++)
				{
					...
					List<RoyalWord_Word_BoardCell> cells = CreateWordBoardCells(level, wordIndex, rowContainer);
		...
		/// </summary>
		private List<RoyalWord_Word_BoardCell> CreateWordBoardCells(RoyalWord_ActiveLevel level, int wordIndex, Transform parent)
		{
			WordData wordData = level.levelData.Words[wordIndex];

			List<RoyalWord_Word_BoardCell> wordCells = new List<RoyalWord_Word_BoardCell>();

			for (int k = 0; k < wordData.Word.Length; k++)
			{
				RoyalWord_Word_BoardCell wordBoardCell = CreateWordBoardCell(parent, wordData.Word[k], wordData.AwardCoins, level.packInfo.color);
		...
		/// </summary>
		private RoyalWord_Word_BoardCell CreateWordBoardCell(Transform parent, char letter, bool hasCoin, Color packColor)
		{
			RectTransform cellContainer = cellContainerPool.GetObject(parent).transform as RectTransform;
			RoyalWord_Word_BoardCell wordBoardCell = wordBoardCellPool.GetObject<RoyalWord_Word_BoardCell>(cellContainer);
			...
			wordBoardCell.SetState(RoyalWord_Word_BoardCell.State.Blank);
			...
			RoyalWord_UIAnimation.DestroyAllAnimations(wordBoardCell.letterText.gameObject);
			RoyalWord_UIAnimation.DestroyAllAnimations(wordBoardCell.letterBackground.gameObject);
		...
		{
			GameObject listObject = new GameObject("board_container", typeof(RectTransform), typeof(RoyalWord_ScaleToFit));
