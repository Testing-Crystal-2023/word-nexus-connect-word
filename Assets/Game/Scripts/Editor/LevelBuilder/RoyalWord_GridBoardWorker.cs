...
namespace Game
{
	public class RoyalWord_GridBoardWorker : RoyalWord_Worker
	{
		#region Classes

		public class RoyalWord_Board
		{
			public int numRows;
			public int numCols;
			public List<RoyalWord_BoardWord> boardWords;
			public char[][] letters;    // Only used when displaying the board in the LevelBuilderWindow
		}

		public class RoyalWord_BoardWord
		{
			public string word;
		...
		private class RoyalWord_Grid
		{
			public List<List<RoyalWord_GridCell>> gridCells;
			public int numRows;
			public int numCols;
			public int rowOrigin;
			public int colOrigin;
			public List<RoyalWord_GridCell> placedLetters;
			public int numPlacedWords;
			public bool hasBonusWord;
			public RoyalWord_GridWord bonusWord;
		}

		private class RoyalWord_GridCell
		{
		...
		private class RoyalWord_CrossLetter
		{
			public char letter = '\0';
			public int offset = 0;
		}

		private class RoyalWord_GridWord
		{
			public string word;
			public List<RoyalWord_GridWordLetter> gridWordLetters;
		...
		private class RoyalWord_GridWordLetter
		{
			public RoyalWord_GridWord gridWord;
			public char letter;
			public int index;
		}
		...
		// Out Values
		public RoyalWord_Board BestBoard { get; set; }
		...
		{
			RoyalWord_Grid grid = CreateGrid(BonusWord);
			List<RoyalWord_GridWord> gridWords = CreateGridWords(Words);
			Dictionary<char, List<RoyalWord_GridWordLetter>> letterMap = CreateLetterMap(gridWords);

			// Get the list of starting GridCells
			List<RoyalWord_GridCell> startingGridCells = GetStartingGridCells(grid);

			// Create an empty list of grid boards, we will fill this with all possible boards
			List<RoyalWord_Board> gridBoards = new List<RoyalWord_Board>();
			...
			else
			{
				...
				{
					RoyalWord_GridCell startingGridCell = startingGridCells[i];

					for (int j = 0; j < gridWords.Count; j++)
					{
						RoyalWord_GridWord gridWord = gridWords[j];
		...
		/// </summary>
		public static void SetBoardLettersArray(RoyalWord_Board board)
		{
			// Initalize the board
			...
			{
				RoyalWord_BoardWord boardWord = board.boardWords[i];
		...
		/// </summary>
		private List<RoyalWord_GridWord> CreateGridWords(List<string> words)
		{
			List<RoyalWord_GridWord> gridWords = new List<RoyalWord_GridWord>();
		...
		/// </summary>
		private Dictionary<char, List<RoyalWord_GridWordLetter>> CreateLetterMap(List<RoyalWord_GridWord> gridWords)
		{
			Dictionary<char, List<RoyalWord_GridWordLetter>> letterMap = new Dictionary<char, List<RoyalWord_GridWordLetter>>();

			for (int i = 0; i < gridWords.Count; i++)
			{
				RoyalWord_GridWord gridWord = gridWords[i];

				for (int j = 0; j < gridWord.gridWordLetters.Count; j++)
				{
					RoyalWord_GridWordLetter gridWordLetter = gridWord.gridWordLetters[j];
					char letter = gridWordLetter.letter;

					if (!letterMap.ContainsKey(letter))
					{
						// Add a new letter list to the map
						letterMap.Add(letter, new List<RoyalWord_GridWordLetter>());
					}
		...
		/// </summary>
		private RoyalWord_Grid CreateGrid(string bonusWord)
		{
			RoyalWord_Grid grid = new RoyalWord_Grid
			{
				...
				numCols = 1,
				numRows = 1,
				gridCells = new List<List<RoyalWord_GridCell>>() { new List<RoyalWord_GridCell>() { CreateGridCell(0, 0) } },
				placedLetters = new List<RoyalWord_GridCell>()
			};

			if (!string.IsNullOrEmpty(bonusWord))
			{
				RoyalWord_GridWord bonusGridWord = CreateGridWord(bonusWord);
		...
		/// </summary>
		private RoyalWord_GridWord CreateGridWord(string word)
		{
			RoyalWord_GridWord gridWord = new RoyalWord_GridWord
			{
				word = word,
				isPlaced = false
			};
		...
		/// </summary>
		private List<RoyalWord_GridWordLetter> CreateGridWordLetters(RoyalWord_GridWord gridWord)
		{
			List<RoyalWord_GridWordLetter> gridWordLetters = new List<RoyalWord_GridWordLetter>();

			for (int i = 0; i < gridWord.word.Length; i++)
			{
				RoyalWord_GridWordLetter gridWordLetter = CreateGridWordLetter(gridWord, i);
		...
		/// </summary>
		private RoyalWord_GridWordLetter CreateGridWordLetter(RoyalWord_GridWord gridWord, int index)
		{
			RoyalWord_GridWordLetter gridWordLetter = new RoyalWord_GridWordLetter
			{
				gridWord = gridWord,
		...
		/// </summary>
		private RoyalWord_GridCell CreateGridCell(int rowOriginOffset, int colOriginOffset, bool canPlaceLetter = true, char letter = (char)0)
		{
			RoyalWord_GridCell gridCell = new RoyalWord_GridCell
			{
		...
		/// </summary>
		private List<RoyalWord_GridCell> GetStartingGridCells(RoyalWord_Grid grid)
		{
			List<RoyalWord_GridCell> startingGridCells = new List<RoyalWord_GridCell>();

			for (int i = 0; i < grid.gridCells.Count; i++)
			{
				for (int j = 0; j < grid.gridCells[i].Count; j++)
				{
					RoyalWord_GridCell gridCell = grid.gridCells[i][j];
		...
		/// </summary>
		private void BeginCreateBoards(RoyalWord_Grid grid,
									   List<RoyalWord_GridWord> gridWords,
									   Dictionary<char, List<RoyalWord_GridWordLetter>> letterMap,
									   List<RoyalWord_Board> gridBoards,
									   RoyalWord_GridWord startingGridWord,
									   int rowIndex,
									   int colIndex,
		...
		/// </summary>
		private void PlaceWordAndCreateBoards(RoyalWord_Grid grid,
											 List<RoyalWord_GridWord> gridWords,
											 Dictionary<char, List<RoyalWord_GridWordLetter>> letterMap,
											 List<RoyalWord_Board> gridBoards,
											 RoyalWord_GridWord gridWord,
											 int rowIndex,
											 int colIndex,
											 Direction direction)
		{
		...
		/// </summary>
		private int PlaceWord(RoyalWord_Grid grid, RoyalWord_GridWord gridWord, int rowIndex, int colIndex, Direction direction)
		{
		...
		/// </summary>
		private bool PlaceLetter(RoyalWord_Grid grid, char letter, int rowIndex, int colIndex)
		{
			...
			RoyalWord_GridCell gridCell = grid.gridCells[rowIndex][colIndex];
		...
		/// </summary>
		private void ClearPlacedLetters(RoyalWord_Grid grid, int numToClear)
		{
			for (int i = 0; i < numToClear; i++)
			{
				int index = grid.placedLetters.Count - 1;

				RoyalWord_GridCell gridCell = grid.placedLetters[index];
		...
		/// </summary>
		private void AddRows(RoyalWord_Grid grid, int numRows, bool atBeginning)
		{
		...
		/// </summary>
		private void AddCols(RoyalWord_Grid grid, int numCols, bool atBeginning)
		{
		...
		/// </summary>
		private void AddRow(RoyalWord_Grid grid, bool insertAtBeginning)
		{
			...
			List<RoyalWord_GridCell> rowGridCells = new List<RoyalWord_GridCell>();
		...
		/// </summary>
		private void AddCol(RoyalWord_Grid grid, bool insertAtBeginning)
		{
			...
			{
				RoyalWord_GridCell gridCell = CreateGridCell(i - grid.rowOrigin, colOriginOffset);
		...
		/// </summary>
		private void CreateBoards(RoyalWord_Grid grid,
								  List<RoyalWord_GridWord> gridWords,
								  Dictionary<char, List<RoyalWord_GridWordLetter>> letterMap,
								  List<RoyalWord_Board> gridBoards)
		{
			...
			for (int i = 0; i < grid.placedLetters.Count; i++)
			{
				...
				RoyalWord_GridCell gridCell = grid.placedLetters[i];
				int rowIndex = GetRowIndex(grid, gridCell);
				int colIndex = GetColIndex(grid, gridCell);

				List<RoyalWord_GridWordLetter> gridWordLetters = GetPlaceableGridWordLetters(letterMap, gridCell.letter);
		...
		/// </summary>
		private void TryPlaceWords(RoyalWord_Grid grid,
								   List<RoyalWord_GridWord> gridWords,
								   Dictionary<char, List<RoyalWord_GridWordLetter>> letterMap,
								   List<RoyalWord_Board> gridBoards,
								   Direction direction,
								   List<RoyalWord_GridWordLetter> gridWordLetters,
								   RoyalWord_GridCell gridCell,
								   int crossWorStartOffset,
								   int crossWordEndOffset,
								   List<int> crossLetterOffsets)
		{
			for (int i = 0; i < gridWordLetters.Count; i++)
			{
				...
				RoyalWord_GridWordLetter gridWordLetter = gridWordLetters[randIndex];
		...
		/// </summary>
		private bool CanPlaceWord(RoyalWord_Grid grid, RoyalWord_GridWordLetter gridWordLetter, Direction direction, int rowIndex, int colIndex, int crossWorStartOffset, int crossWordEndOffset, List<int> crossLetterOffsets)
		{
			int gridIndex = (direction == Direction.Vertical ? rowIndex : colIndex);

			RoyalWord_GridWord gridWord = gridWordLetter.gridWord;
		...
		/// </summary>
		private bool CanPlaceAnyWordVertically(RoyalWord_Grid grid, int rowIndex, int colIndex, out int crossWordStartOffset, out int crossWordEndOffset, out List<int> crossLetterOffsets)
		{
		...
		/// </summary>
		private bool CanPlaceAnyWordHorizontally(RoyalWord_Grid grid, int rowIndex, int colIndex, out int crossWordStartOffset, out int crossWordEndOffset, out List<int> crossLetterOffsets)
		{
			crossWordStartOffset = -1;
			crossWordEndOffset = -1;
			crossLetterOffsets = new List<int>();
		...
		/// </summary>
		private List<RoyalWord_GridWordLetter> GetPlaceableGridWordLetters(Dictionary<char, List<RoyalWord_GridWordLetter>> letterMap, char letter)
		{
			List<RoyalWord_GridWordLetter> gridWordLetters = letterMap.ContainsKey(letter) ? letterMap[letter] : null;
			List<RoyalWord_GridWordLetter> placeableWords = new List<RoyalWord_GridWordLetter>();

			for (int i = 0; i < gridWordLetters.Count; i++)
			{
				RoyalWord_GridWordLetter gridWordLetter = gridWordLetters[i];
		...
		/// </summary>
		private int GetRowIndex(RoyalWord_Grid grid, RoyalWord_GridCell gridCell)
		{
		...
		/// </summary>
		private int GetColIndex(RoyalWord_Grid grid, RoyalWord_GridCell gridCell)
		{
		...
		/// </summary>
		private bool IsPartOfVerticalWord(RoyalWord_Grid grid, int rowIndex, int colIndex)
		{
		...
		/// Checks if the cell at the given index is part of a vertical word
		/// </summary>
		private bool IsPartOfHorizontalWord(RoyalWord_Grid grid, int rowIndex, int colIndex)
		{
		...
		/// </summary>
		private bool HasLetter(RoyalWord_Grid grid, int rowIndex, int colIndex)
		{
		...
		/// Checks if the cell at the given row/col index has a letter placed on it
		/// </summary>
		private bool CanPlace(RoyalWord_Grid grid, int rowIndex, int colIndex)
		{
		...
		/// </summary>
		private RoyalWord_Board CreateBoard(RoyalWord_Grid grid, List<RoyalWord_GridWord> gridWords)
		{
			RoyalWord_Board board = new RoyalWord_Board();

			board.boardWords = new List<RoyalWord_BoardWord>();
			...
			{
				RoyalWord_GridWord gridWord = (i == -1 ? grid.bonusWord : gridWords[i]);
				RoyalWord_BoardWord boardWord = new RoyalWord_BoardWord();
		...
		/// </summary>
		private void NormalizeBoard(RoyalWord_Board board)
		{
		...
		/// </summary>
		private RoyalWord_Board GetBestBoard(List<RoyalWord_Board> boards)
		{
			if (boards.Count == 0)
			{
				return null;
			...
			List<RoyalWord_Board> bestBoards = new List<RoyalWord_Board>() { boards[0] };
			float bestValue = CalculateValue(boards[0]);

			for (int i = 0; i < boards.Count; i++)
			{
				RoyalWord_Board board = boards[i];
				...
				{
					bestBoards = new List<RoyalWord_Board>() { board };
		...
		/// </summary>
		private float CalculateValue(RoyalWord_Board board)
		{
		...
		/// </summary>
		private void PrintBoard(RoyalWord_Board board)
		{
			char[][] letters = new char[board.numRows][];
		...
		/// Prints the grid cells
		/// </summary>
		private void PrintBoardInfo(RoyalWord_Board board)
		{
			string print = "";
		...
		/// Prints the grid cells
		/// </summary>
		private void DebugPrintGridCells(RoyalWord_Grid grid)
		{
			string print = "";
		...
		/// Prints the grid cells
		/// </summary>
		private void DebugPrintRowOffsets(RoyalWord_Grid grid)
		{
			string print = "";
		...
		/// Prints the grid cells
		/// </summary>
		private void DebugPrintColOffsets(RoyalWord_Grid grid)
		{
			string print = "";
