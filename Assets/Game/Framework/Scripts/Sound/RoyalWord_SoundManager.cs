...
namespace Game
{
	public class RoyalWord_SoundManager : SingletonComponent<RoyalWord_SoundManager>, RoyalWord_ISaveable
	{
		#region Classes

		[System.Serializable]
		private class RoyalWord_SoundInfo
		{
			public string id = "";
		...
		private class RoyalWord_PlayingSound
		{
			public RoyalWord_SoundInfo soundInfo = null;
		...
		[SerializeField] private List<RoyalWord_SoundInfo> soundInfos = null;

		#endregion

		#region Member Variables

		private List<RoyalWord_PlayingSound> playingAudioSources;
		private List<RoyalWord_PlayingSound> loopingAudioSources;
		...
		protected override void Awake()
		{
			...
			playingAudioSources = new List<RoyalWord_PlayingSound>();
			loopingAudioSources = new List<RoyalWord_PlayingSound>();
		...
		private void Start()
		{
			...
			{
				RoyalWord_SoundInfo soundInfo = soundInfos[i];
		...
		{
			RoyalWord_SoundInfo soundInfo = GetSoundInfo(id);
			...
			RoyalWord_PlayingSound playingSound = new RoyalWord_PlayingSound();
		...
		/// </summary>
		private void StopAllSounds(string id, List<RoyalWord_PlayingSound> playingSounds)
		{
			for (int i = 0; i < playingSounds.Count; i++)
			{
				RoyalWord_PlayingSound playingSound = playingSounds[i];
		...
		/// </summary>
		private void StopAllSounds(SoundType type, List<RoyalWord_PlayingSound> playingSounds)
		{
			for (int i = 0; i < playingSounds.Count; i++)
			{
				RoyalWord_PlayingSound playingSound = playingSounds[i];
		...
		private RoyalWord_SoundInfo GetSoundInfo(string id)
		{
			for (int i = 0; i < soundInfos.Count; i++)
			{
