...
namespace Game
{
	public class RoyalWord_Word_BoardGrid : RoyalWord_WordBoard
	{
		...
		private RoyalWord_ObjectPool cellPlacementPool;
		...
		private List<List<RoyalWord_Word_BoardCell>> wordBoardCells;
		...
		public override void Initialize()
		{
			...
			wordBoardCells = new List<List<RoyalWord_Word_BoardCell>>();
			...
			cellPlacementPool = new RoyalWord_ObjectPool(cellPlacementTemplate.gameObject, 9, poolContainer);
			wordBoardCellPool = new RoyalWord_ObjectPool(wordBoardCellPrefab.gameObject, 9, poolContainer);
		...
		protected override List<RoyalWord_Word_BoardCell> GetWordGridCells(WordData wordData)
		{
			...
			List<RoyalWord_Word_BoardCell> cells = new List<RoyalWord_Word_BoardCell>();
		...
		/// </summary>
		private void SetupGridGameObject(RoyalWord_LevelData levelData)
		{
		...
		/// </summary>
		private void CreateCellPlacements(RoyalWord_LevelData levelData)
		{
		...
		/// </summary>
		private void CreateWordGridCells(RoyalWord_ActiveLevel level)
		{
		...
		/// </summary>
		private void CreateEmptyWordGridCellsList(RoyalWord_LevelData levelData)
		{
			for (int i = 0; i < levelData.BoardRows; i++)
			{
				List<RoyalWord_Word_BoardCell> row = new List<RoyalWord_Word_BoardCell>();
		...
		/// </summary>
		private void CreateWordGridCells(RoyalWord_ActiveLevel level, WordData wordData)
		{
			string word = wordData.Word;
			int letterIndex = 0;
			...
			for (int i = wordData.BoardRowStartIndex; i <= wordData.BoardRowEndIndex; i++)
			{
				for (int j = wordData.BoardColStartIndex; j <= wordData.BoardColEndIndex; j++)
				{
					RoyalWord_Word_BoardCell wordBoardCell = CreateWordGridCell(i, j);
		...
		/// </summary>
		private RoyalWord_Word_BoardCell CreateWordGridCell(int row, int col)
		{
			if (wordBoardCells[row][col] == null)
			{
				GameObject cellGameObject = cellPlacements[row][col];
				RoyalWord_Word_BoardCell wordBoardCell = wordBoardCellPool.GetObject<RoyalWord_Word_BoardCell>(cellGameObject.transform);
