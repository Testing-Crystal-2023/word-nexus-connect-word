...
namespace Game
{
	...
	public class RoyalWord_Game_Controller_Editor : Editor
	{
		#region Enums
		...
		private ListAction DrawCategoryInfo(SerializedProperty categoryInfoProperty, int index, List<int> parentIndices)
		{
			...
			if (categoryInfoProperty.isExpanded)
			{
				...
				{
					List<RoyalWord_PackInfo> packInfos = (target as RoyalWord_GameController).PackInfos;
					List<RoyalWord_CategoryInfo> categoryInfos = packInfos[parentIndices[0]].categoryInfos;
					RoyalWord_CategoryInfo categoryInfo = categoryInfos[index];
		...
		{
			List<RoyalWord_PackInfo> packInfos = (target as RoyalWord_GameController).PackInfos;
			RoyalWord_PackInfo packItemToMove = packInfos[fromIndex];
		...
		private void MoveCategoryInfoItem(int fromIndex, int toIndex, List<int> parentIndices)
		{
			List<RoyalWord_PackInfo> packInfos = (target as RoyalWord_GameController).PackInfos;
			List<RoyalWord_CategoryInfo> categoryInfos = packInfos[parentIndices[0]].categoryInfos;
			RoyalWord_CategoryInfo categoryInfo = categoryInfos[fromIndex];
		...
		private void AddSelectedObjectsToLevelFiles(RoyalWord_CategoryInfo categoryInfo)
		{
