...
namespace WordCross
{
	public class WordNexus_LevelBuilderWindow : EditorWindow
	{
		#region Classes

		private class WordNexus_PreviewLetter
		{
			public char letter;
			public bool isNew;
		...
		private Dictionary<string, List<WordNexus_RankedWord>> wordDictionary;
		private Dictionary<int, List<WordNexus_RankedWord>> wordsByLength;
		...
		private List<WordNexus_PreviewLetter> previewLetters;         // Sorted letters that contains both choosenSortedLetters and the selectedWord
		private List<WordNexus_RankedWord> possibleWords;
		private WordNexus_GridBoardWorker.WordNexus_Board board;
		private WordNexus_GridBoardWorker gridBoardWorker;
		private WordNexus_BatchLevelCreationWorker batchLevelCreationWorker;
		...
		private Dictionary<string, List<WordNexus_RankedWord>> WordDictionary { get { return (wordDictionary == null) ? wordDictionary = new Dictionary<string, List<WordNexus_RankedWord>>() : wordDictionary; } }
		private Dictionary<int, List<WordNexus_RankedWord>> WordsByLength { get { return (wordsByLength == null) ? wordsByLength = new Dictionary<int, List<WordNexus_RankedWord>>() : wordsByLength; } }
		private List<WordNexus_PreviewLetter> PreviewLetters { get { return (previewLetters == null) ? previewLetters = new List<WordNexus_PreviewLetter>() : previewLetters; } }
		...
		private List<WordNexus_RankedWord> PossibleWords { get { return (possibleWords == null) ? possibleWords = new List<WordNexus_RankedWord>() : possibleWords; } set { possibleWords = value; } }
		...
		/// </summary>
		private Texture2D LineTexture { get { return (lineTexture != null) ? lineTexture : (lineTexture = WordNexus_Utilities.CreateTexture(1, 1, GUI.skin.label.normal.textColor)); } }
		...
		private void DrawLevelWords(float width, float height)
		{
			...
			while (lettersPerRow > 0 && letterIndex < PreviewLetters.Count)
			{
				...
				{
					WordNexus_PreviewLetter previewLetter = PreviewLetters[letterIndex];
		...
		private void DrawExportSettings()
		{
			...
			string folderPath = WordNexus_LevelBuilderUtilities.GetOutputFolderPath(outputFolder);
		...
		private void ProcessWordFile()
		{
			...
			for (int i = 0; i < words.Length; i++)
			{
				...
				string sortedWord = WordNexus_LevelBuilderUtilities.SortAlphabetically(wordStr);
				WordNexus_RankedWord word = new WordNexus_RankedWord(wordStr, i);

				if (!WordDictionary.ContainsKey(sortedWord))
				{
					WordDictionary.Add(sortedWord, new List<WordNexus_RankedWord>());
				}
				...
				{
					WordsByLength.Add(wordStr.Length, new List<WordNexus_RankedWord>());
				}
		...
		{
			choosenSortedLetters = WordNexus_LevelBuilderUtilities.GetLetters(ChoosenWords);
		}
		...
		private void UpdatePreviewLetters()
		{
			...
			for (int i = 0; i < customWord.Length; i++)
			{
				...
				WordNexus_PreviewLetter previewLetter = new WordNexus_PreviewLetter();
			...
			PreviewLetters.Sort((WordNexus_PreviewLetter x, WordNexus_PreviewLetter y) =>
				{
		...
		private void UpdatePossibleWords()
		{
			...
			{
				PossibleWords = WordNexus_LevelBuilderUtilities.GetWordsThatFit(choosenSortedLetters, letterCount, WordDictionary, WordsByLength);
			}
		...
		private void UpdateBoard()
		{
			...
			{
				// Create a new worker
				gridBoardWorker = new WordNexus_GridBoardWorker();
		...
		{
			batchLevelCreationWorker = new WordNexus_BatchLevelCreationWorker();
			...
			batchLevelCreationWorker.LoadFromFolderPath = WordNexus_LevelBuilderUtilities.GetFolderPath(loadWordsFromFolder);
			...
			batchLevelCreationWorker.OutputFolderPath = WordNexus_LevelBuilderUtilities.GetOutputFolderPath(outputFolder);
		...
		private void ExportLevel(string folderPath)
		{
			...
			bool fileCreated = WordNexus_LevelBuilderUtilities.ExportLevelFile(choosenSortedLetters, boardType, board, ChoosenWords, ChoosenWordsStar, false, 0, folderPath, filename, false);
			...
			if (!fileCreated)
			{
				if (EditorUtility.DisplayDialog("File Already Exists", "The level file already exists in the given folder, would you like to overwrite it?", "Yes", "No"))
				{
					...
					fileCreated = WordNexus_LevelBuilderUtilities.ExportLevelFile(choosenSortedLetters, boardType, board, ChoosenWords, ChoosenWordsStar, false, 0, folderPath, filename, true);
		...
		private void ImportLevelFile(TextAsset levelFile)
		{
			...
			BoardType newBoardType;
			WordNexus_GridBoardWorker.WordNexus_Board newBoard;

			bool loaded = WordNexus_LevelBuilderUtilities.LoadLevelFile(levelFile.text, out letters, out wordsInLevel, out wordIsStarred, out newBoardType, out newBoard);
		...
		private void SortPossibleWords()
		{
			...
			{
				case SortType.Rank:
					PossibleWords.Sort((WordNexus_RankedWord word1, WordNexus_RankedWord word2) =>
					{
