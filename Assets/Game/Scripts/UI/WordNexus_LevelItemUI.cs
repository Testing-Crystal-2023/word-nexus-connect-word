...
namespace Game
{
	public class WordNexus_LevelItemUI : WordNexus_ClickableListItem_
	{
		#region Classes

		[System.Serializable]
		private class WordNexus_LevelPreviewSetting
		{
		...
		[SerializeField] private WordNexus_LevelPreviewText levelPreview = null;
		[SerializeField] private string currentLevelAnimationId = "";
		[SerializeField] private List<WordNexus_LevelPreviewSetting> letterPreviewSettings = null;
		...
		public void Setup(WordNexus_LevelData levelData, Color packColor)
		{
			if (!initialized)
			{
				Initialize();
			...
			WordNexus_LevelPreviewSetting levelPreviewSetting = GetLevelPreviewSetting(levelData.Letters.Length);
			...
			{
				WordNexus_UIAnimation.StopAllById(gameObject, currentLevelAnimationId);
		...
		private WordNexus_LevelPreviewSetting GetLevelPreviewSetting(int letterCount)
		{
			for (int i = 0; i < letterPreviewSettings.Count; i++)
			{
				WordNexus_LevelPreviewSetting levelPreviewSetting = letterPreviewSettings[i];
