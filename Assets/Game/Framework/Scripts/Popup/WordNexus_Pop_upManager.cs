using System.Collections;
using System.Collections.Generic;
using UnityEngine;

namespace Game
{
	public class WordNexus_Pop_upManager : SingletonComponent<WordNexus_Pop_upManager>
	{
		#region Classes

		[System.Serializable]
		private class WordNexus_PopupInfo
		{
			[Tooltip("The popups id, used to show the popup. Should be unique between all other popups.")]
			public string popupId = "";

			[Tooltip("The Popup component to show.")]
			public WordNexus_Popup popup = null;
		}

		#endregion

		#region Inspector Variables

		[SerializeField] private List<WordNexus_PopupInfo> popupInfos = null;

		#endregion

		#region Member Variables

		private List<WordNexus_Popup> activePopups;

		#endregion

		#region Unity Methods

		protected override void Awake()
		{
			base.Awake();

			activePopups = new List<WordNexus_Popup>();

			for (int i = 0; i < popupInfos.Count; i++)
			{
				popupInfos[i].popup.Initialize();
			}
		}

		#endregion

		#region Public Methods

		public void Show(string id)
		{
			Show(id, null, null);
		}

		public void ShowNoAd(string id)
		{
			Show(id, null, null);
			Advertisements.Instance.HideBanner();
		}

		public void Show(string id, object[] inData)
		{
			Show(id, inData, null);
		}

		public void Show(string id, object[] inData, WordNexus_Popup.PopupClosed popupClosed)
		{
			WordNexus_Popup popup = GetPopupById(id);

			if (popup != null)
			{
				activePopups.Add(popup);

				popup.Show(inData, popupClosed);
			}
			else
			{
				Debug.LogErrorFormat("[PopupController] Popup with id {0} does not exist", id);
			}
		}

		public bool CloseActivePopup()
		{
			if (activePopups.Count > 0)
			{
				int index = activePopups.Count - 1;

				if (activePopups[index].CanAndroidBackClosePopup)
				{
					activePopups[index].Hide(true);
				}

				return true;
			}

			return false;
		}

		public void OnPopupHiding(WordNexus_Popup popup)
		{
			for (int i = activePopups.Count - 1; i >= 0; i--)
			{
				if (popup == activePopups[i])
				{
					activePopups.RemoveAt(i);

					break;
				}
			}
		}

		#endregion

		#region Private Methods

		private WordNexus_Popup GetPopupById(string id)
		{
			for (int i = 0; i < popupInfos.Count; i++)
			{
				if (id == popupInfos[i].popupId)
				{
					return popupInfos[i].popup;
				}
			}

			return null;
		}

		#endregion
	}
}