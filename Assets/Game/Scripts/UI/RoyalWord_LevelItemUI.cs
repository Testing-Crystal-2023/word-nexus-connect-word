...
namespace Game
{
	public class RoyalWord_LevelItemUI : RoyalWord_ClickableListItem
	{
		#region Classes

		[System.Serializable]
		private class RoyalWord_LevelPreviewSetting
		{
		...
		[SerializeField] private RoyalWord_LevelPreviewText levelPreview = null;
		[SerializeField] private string currentLevelAnimationId = "";
		[SerializeField] private List<RoyalWord_LevelPreviewSetting> letterPreviewSettings = null;
		...
		public void Setup(RoyalWord_LevelData levelData, Color packColor)
		{
			if (!initialized)
			{
				Initialize();
			...
			RoyalWord_LevelPreviewSetting levelPreviewSetting = GetLevelPreviewSetting(levelData.Letters.Length);
			...
			{
				RoyalWord_UIAnimation.PlayAllById(gameObject, currentLevelAnimationId);
			}
			else
			{
				RoyalWord_UIAnimation.StopAllById(gameObject, currentLevelAnimationId);
		...
		private RoyalWord_LevelPreviewSetting GetLevelPreviewSetting(int letterCount)
		{
			for (int i = 0; i < letterPreviewSettings.Count; i++)
			{
				RoyalWord_LevelPreviewSetting levelPreviewSetting = letterPreviewSettings[i];
