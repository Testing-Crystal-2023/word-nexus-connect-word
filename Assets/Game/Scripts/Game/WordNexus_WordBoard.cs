...
namespace Game
{
	public abstract class WordNexus_WordBoard : WordNexus_UIMonoBehaviour
	{
		...
		/// </summary>
		public abstract void Setup(WordNexus_ActiveLevel level);
		...
		/// </summary>
		protected abstract List<WordNexus_Word_BoardCell> GetWordGridCells(WordData wordData);
		...
		/// </summary>
		protected void ShowFoundWords(WordNexus_ActiveLevel level)
		{
		...
		protected void ShowWord(WordData levelWordData, bool animate, bool awardCoins)
		{
			...
			{
				WordNexus_Word_BoardCell wordBoardCell = wordGridCells[i];

				// Check if we need to award a coin for this cell
				if (awardCoins && wordBoardCell.HasCoin && wordBoardCell.CurrentState == WordNexus_Word_BoardCell.State.Blank)
				{
					coinsToAward.Add(wordBoardCell.coinObject.transform as RectTransform);
				}

				SetWordBoardState(wordBoardCell, WordNexus_Word_BoardCell.State.Found, animate, (float)i * delayBetweenCells);
			}
		...
		protected IEnumerator ShakeWordOnBoard(WordData levelWordData)
		{
			...
			List<WordNexus_Word_BoardCell> boardCells = GetWordGridCells(levelWordData);

			for (int i = 0; i < numShakes; i++)
			{
				for (int j = 0; j < boardCells.Count; j++)
				{
					WordNexus_Word_BoardCell boardCell = boardCells[j];
					...
					{
						WordNexus_UIAnimation.PositionX(boardCell.transform as RectTransform, toPos, shakeSpeed).Play();
					}
					else
					{
						WordNexus_UIAnimation.PositionY(boardCell.transform as RectTransform, toPos, shakeSpeed).Play();
					}
				}
		...
		protected void ShowLetterForHint(WordData levelWordData, int index, bool animate, bool awardCoins)
		{
			...
			SetWordBoardState(wordBoardCell, WordNexus_Word_BoardCell.State.Hint, animate, 0);
		}

		/// <summary>
		/// Sets the state on the WOrdBoardCell and animates it
		/// </summary>
		protected void SetWordBoardState(WordNexus_Word_BoardCell wordBoardCell, WordNexus_Word_BoardCell.State state, bool animate, float startDelay)
		{
			WordNexus_Word_BoardCell.State fromState = wordBoardCell.CurrentState;

			if (fromState != WordNexus_Word_BoardCell.State.Found)
			{
		...
		/// </summary>
		protected void AnimateCell(WordNexus_Word_BoardCell wordBoardCell, WordNexus_Word_BoardCell.State fromState, WordNexus_Word_BoardCell.State toState, float startDelay)
		{
			if (fromState == WordNexus_Word_BoardCell.State.Blank && toState == WordNexus_Word_BoardCell.State.Hint)
			{
				FadeInLetterText(wordBoardCell, startDelay);
				ScaleLetterText(wordBoardCell, startDelay);
			}

			if (fromState == WordNexus_Word_BoardCell.State.Blank && toState == WordNexus_Word_BoardCell.State.Found)
			{
				FadeInLetterText(wordBoardCell, startDelay);
			...
			if (fromState == WordNexus_Word_BoardCell.State.Hint && toState == WordNexus_Word_BoardCell.State.Found)
			{
				FadeInLetterBackground(wordBoardCell, startDelay);
		...
		/// </summary>
		protected void FadeInLetterText(WordNexus_Word_BoardCell wordBoardCell, float startDelay)
		{
			...
			WordNexus_UIAnimation anim = WordNexus_UIAnimation.Color(wordBoardCell.letterText, fromColor, toColor, animationDuration);
		...
		/// </summary>
		protected void FadeInLetterBackground(WordNexus_Word_BoardCell wordBoardCell, float startDelay)
		{
			...
			WordNexus_UIAnimation anim = WordNexus_UIAnimation.Color(wordBoardCell.letterBackground, fromColor, toColor, animationDuration);
			SetupAndPlay(anim, startDelay);
		}
		...
		/// Fades in the cells letter text
		/// </summary>
		protected void FadeLetterTextColor(WordNexus_Word_BoardCell wordBoardCell, Color fromColor, Color toColor, float startDelay)
		{
			WordNexus_UIAnimation anim = WordNexus_UIAnimation.Color(wordBoardCell.letterText, fromColor, toColor, animationDuration);
			SetupAndPlay(anim, startDelay);
		}
		...
		/// </summary>
		protected void ScaleLetterText(WordNexus_Word_BoardCell wordBoardCell, float startDelay)
		{
			WordNexus_UIAnimation anim;

			// Play the scale x animation
			anim = WordNexus_UIAnimation.ScaleX(wordBoardCell.letterText.transform as RectTransform, 0f, 1f, animationDuration);
			SetupAndPlay(anim, startDelay);

			// Play the scale y animation
			anim = WordNexus_UIAnimation.ScaleY(wordBoardCell.letterText.transform as RectTransform, 0f, 1f, animationDuration);
			SetupAndPlay(anim, startDelay);
		}

		protected void SetupAndPlay(WordNexus_UIAnimation anim, float startDelay)
		{
			anim.style = WordNexus_UIAnimation.Style.Custom;
