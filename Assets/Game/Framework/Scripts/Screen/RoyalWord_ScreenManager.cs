...
namespace Game
{
	public class RoyalWord_ScreenManager : SingletonComponent<RoyalWord_ScreenManager>
	{
		...
		[SerializeField] private List<RoyalWord_Screen> screens = null;
		...
		private RoyalWord_Screen currentScreen;
		...
		private void Start()
		{
			...
			{
				RoyalWord_Screen screen = screens[i];
		...
		private void Update()
		{
			if (Input.GetKeyDown(KeyCode.Escape))
			{
				...
				if (!RoyalWord_Pop_upManager.Instance.CloseActivePopup())
				{
		...
		private void Show(string screenId, bool back, bool immediate)
		{
			...
			RoyalWord_Screen screen = GetScreenById(screenId);
		...
		private RoyalWord_Screen GetScreenById(string id)
		{
			for (int i = 0; i < screens.Count; i++)
			{
				if (id == screens[i].Id)
