...
namespace Game
{
	public abstract class RoyalWord_WordBoard : RoyalWord_UIMonoBehaviour
	{
		...
		/// </summary>
		public abstract void Setup(RoyalWord_ActiveLevel level);
		...
		/// </summary>
		protected abstract List<RoyalWord_Word_BoardCell> GetWordGridCells(WordData wordData);
		...
		/// </summary>
		protected void ShowFoundWords(RoyalWord_ActiveLevel level)
		{
		...
		protected void ShowWord(WordData levelWordData, bool animate, bool awardCoins)
		{
			...
			List<RoyalWord_Word_BoardCell> wordGridCells = GetWordGridCells(levelWordData);

			List<RectTransform> coinsToAward = new List<RectTransform>();

			for (int i = 0; i < wordGridCells.Count; i++)
			{
				RoyalWord_Word_BoardCell wordBoardCell = wordGridCells[i];

				// Check if we need to award a coin for this cell
				if (awardCoins && wordBoardCell.HasCoin && wordBoardCell.CurrentState == RoyalWord_Word_BoardCell.State.Blank)
				{
					coinsToAward.Add(wordBoardCell.coinObject.transform as RectTransform);
				}

				SetWordBoardState(wordBoardCell, RoyalWord_Word_BoardCell.State.Found, animate, (float)i * delayBetweenCells);
			}
		...
		protected IEnumerator ShakeWordOnBoard(WordData levelWordData)
		{
			...
			List<RoyalWord_Word_BoardCell> boardCells = GetWordGridCells(levelWordData);

			for (int i = 0; i < numShakes; i++)
			{
				for (int j = 0; j < boardCells.Count; j++)
				{
					RoyalWord_Word_BoardCell boardCell = boardCells[j];
					...
					{
						RoyalWord_UIAnimation.PositionX(boardCell.transform as RectTransform, toPos, shakeSpeed).Play();
					}
					else
					{
						RoyalWord_UIAnimation.PositionY(boardCell.transform as RectTransform, toPos, shakeSpeed).Play();
					}
				}
		...
		{
			List<RoyalWord_Word_BoardCell> wordGridCells = GetWordGridCells(levelWordData);
			RoyalWord_Word_BoardCell wordBoardCell = wordGridCells[index];

			if (awardCoins && wordBoardCell.HasCoin && wordBoardCell.CurrentState == RoyalWord_Word_BoardCell.State.Blank && RoyalWord_CoinController.Exists())
			{
				AwardCoins(new List<RectTransform>() { wordBoardCell.coinObject.transform as RectTransform });
			}

			SetWordBoardState(wordBoardCell, RoyalWord_Word_BoardCell.State.Hint, animate, 0);
		}

		/// <summary>
		/// Sets the state on the WOrdBoardCell and animates it
		/// </summary>
		protected void SetWordBoardState(RoyalWord_Word_BoardCell wordBoardCell, RoyalWord_Word_BoardCell.State state, bool animate, float startDelay)
		{
			RoyalWord_Word_BoardCell.State fromState = wordBoardCell.CurrentState;

			if (fromState != RoyalWord_Word_BoardCell.State.Found)
			{
		...
		/// </summary>
		protected void AnimateCell(RoyalWord_Word_BoardCell wordBoardCell, RoyalWord_Word_BoardCell.State fromState, RoyalWord_Word_BoardCell.State toState, float startDelay)
		{
			if (fromState == RoyalWord_Word_BoardCell.State.Blank && toState == RoyalWord_Word_BoardCell.State.Hint)
			{
				FadeInLetterText(wordBoardCell, startDelay);
				ScaleLetterText(wordBoardCell, startDelay);
			}

			if (fromState == RoyalWord_Word_BoardCell.State.Blank && toState == RoyalWord_Word_BoardCell.State.Found)
			{
				FadeInLetterText(wordBoardCell, startDelay);
			...
			if (fromState == RoyalWord_Word_BoardCell.State.Hint && toState == RoyalWord_Word_BoardCell.State.Found)
			{
				FadeInLetterBackground(wordBoardCell, startDelay);
		...
		/// </summary>
		protected void FadeInLetterText(RoyalWord_Word_BoardCell wordBoardCell, float startDelay)
		{
			...
			RoyalWord_UIAnimation anim = RoyalWord_UIAnimation.Color(wordBoardCell.letterText, fromColor, toColor, animationDuration);
		...
		/// </summary>
		protected void FadeInLetterBackground(RoyalWord_Word_BoardCell wordBoardCell, float startDelay)
		{
			...
			RoyalWord_UIAnimation anim = RoyalWord_UIAnimation.Color(wordBoardCell.letterBackground, fromColor, toColor, animationDuration);
			SetupAndPlay(anim, startDelay);
		}
		...
		/// Fades in the cells letter text
		/// </summary>
		protected void FadeLetterTextColor(RoyalWord_Word_BoardCell wordBoardCell, Color fromColor, Color toColor, float startDelay)
		{
			RoyalWord_UIAnimation anim = RoyalWord_UIAnimation.Color(wordBoardCell.letterText, fromColor, toColor, animationDuration);
			SetupAndPlay(anim, startDelay);
		}
		...
		/// </summary>
		protected void ScaleLetterText(RoyalWord_Word_BoardCell wordBoardCell, float startDelay)
		{
			RoyalWord_UIAnimation anim;

			// Play the scale x animation
			anim = RoyalWord_UIAnimation.ScaleX(wordBoardCell.letterText.transform as RectTransform, 0f, 1f, animationDuration);
			SetupAndPlay(anim, startDelay);

			// Play the scale y animation
			anim = RoyalWord_UIAnimation.ScaleY(wordBoardCell.letterText.transform as RectTransform, 0f, 1f, animationDuration);
			SetupAndPlay(anim, startDelay);
		}

		protected void SetupAndPlay(RoyalWord_UIAnimation anim, float startDelay)
		{
