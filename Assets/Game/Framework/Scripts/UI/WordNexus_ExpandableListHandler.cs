...
namespace Game
{
	public class WordNexus_ExpandableListHandler<T>
	{
		#region Classes

		private class WordNexus_Animation
		{
		...
		private WordNexus_ExpandableListItem<T> listItemPrefab;
		...
		public System.Action<WordNexus_ExpandableListItem<T>> OnItemCreated { get; set; }
		...
		public WordNexus_ExpandableListHandler(List<T> dataObjects, WordNexus_ExpandableListItem<T> listItemPrefab, RectTransform listContainer, ScrollRect listScrollRect, float expandAnimDuration)
		{
		...
		public void CollapseListItem(int index, bool isExpandingNewItem = false)
		{
			...
			animation.OnAnimationFinished += (GameObject obj) =>
			{
				...
				{
					placeholder.GetChild(0).GetComponent<WordNexus_ExpandableListItem<T>>().Collapsed();
				}
			};
		...
		public void Refresh()
		{
			for (int i = topItemIndex; i <= bottomItemIndex; i++)
			{
				...
				if (placeholder.childCount == 1)
				{
					WordNexus_ExpandableListItem<T> listItem = placeholder.GetChild(0).GetComponent<WordNexus_ExpandableListItem<T>>();
		...
		private void AddListItem(int index, RectTransform placeholder, bool addingOnTop)
		{
			...
			WordNexus_ExpandableListItem<T> listItem = listItemPool.GetObject<WordNexus_ExpandableListItem<T>>(placeholder, out itemInstantiated);
		...
		private void SetExpanded(RectTransform placeholder, bool isExpanded)
		{
			if (placeholder.childCount == 1)
			{
				placeholder.GetChild(0).GetComponent<WordNexus_ExpandableListItem<T>>().IsExpanded = isExpanded;
			}
		}
