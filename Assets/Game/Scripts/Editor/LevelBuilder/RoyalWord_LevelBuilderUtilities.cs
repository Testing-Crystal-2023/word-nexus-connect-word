...
namespace Game
{
	public class RoyalWord_RankedWord
	{
		public string word;
		public int rank;

		public RoyalWord_RankedWord(string word, int rank)
		{
			this.word = word;
			this.rank = rank;
		}
	...
	public static class RoyalWord_LevelBuilderUtilities
	{
		...
		/// </summary>
		public static List<RoyalWord_RankedWord> GetWordsThatFit(string letters, int length, Dictionary<string, List<RoyalWord_RankedWord>> wordDictionary, Dictionary<int, List<RoyalWord_RankedWord>> wordsByLength)
		{
			string sortedLetters = SortAlphabetically(letters);
			List<string> sortedLetterCombinations = GetLetterCombinations(sortedLetters);
			List<RoyalWord_RankedWord> words = new List<RoyalWord_RankedWord>();
			...
			if (extraLetters > 0)
			{
				...
				for (int i = letters.Length + 1; i <= length; i++)
				{
					...
					{
						List<RoyalWord_RankedWord> wordsToTry = wordsByLength[wordLength];
		...
		public static bool ExportLevelFile(string letters,
										   BoardType boardType,
										   RoyalWord_GridBoardWorker.RoyalWord_Board board,
		...
		/// </summary>
		public static bool LoadLevelFile(string contents, out string letters, out List<string> wordsInLevel, out List<bool> wordIsStarred, out BoardType boardType, out RoyalWord_GridBoardWorker.RoyalWord_Board board)
		{
			letters = "";
			...
			try
			{
				...
				{
					board = new RoyalWord_GridBoardWorker.RoyalWord_Board();
					board.numRows = levelJson["rowCount"].AsInt;
					board.numCols = levelJson["colCount"].AsInt;
					board.boardWords = new List<RoyalWord_GridBoardWorker.RoyalWord_BoardWord>();
				}
				...
				foreach (JSONNode wordJson in wordsJson)
				{
					...
					if (boardType == BoardType.Grid)
					{
						RoyalWord_GridBoardWorker.RoyalWord_BoardWord boardWord = new RoyalWord_GridBoardWorker.RoyalWord_BoardWord();
				...
				if (board != null)
				{
					RoyalWord_GridBoardWorker.SetBoardLettersArray(board);
				}
		...
		private static List<RoyalWord_RankedWord> GetWordsThatFit(List<RoyalWord_RankedWord> wordsToTry, Dictionary<char, int> letterDictionary, int minSharedLetters)
		{
			List<RoyalWord_RankedWord> words = new List<RoyalWord_RankedWord>();
												  ...
												  out string levelId)
		{
			...
			if (boardType == BoardType.Grid && board != null)
			{
				...
				{
					RoyalWord_GridBoardWorker.RoyalWord_BoardWord boardWord = board.boardWords[i];
			...
			levelId = GetLevelId(RoyalWord_Utilities.ConvertToJsonString(json));
			...
			return RoyalWord_Utilities.ConvertToJsonString(json);
		}
		...
		/// </summary>
		private static List<int> GetCoinIndicies(RoyalWord_GridBoardWorker.RoyalWord_Board board, RoyalWord_GridBoardWorker.RoyalWord_BoardWord boardWord, List<string> choosenWords, List<bool> starredChoosenWords)
		{
			...
			for (int i = 0; i < boardWord.word.Length; i++)
			{
				...
				else
				{
					...
					RoyalWord_GridBoardWorker.RoyalWord_BoardWord crossingBoardWord = GetCrossingWord(board, rowIndex, colIndex, !boardWord.isVertical);
		...
		/// </summary>
		private static RoyalWord_GridBoardWorker.RoyalWord_BoardWord GetCrossingWord(RoyalWord_GridBoardWorker.RoyalWord_Board board, int rowIndex, int colIndex, bool vertical)
		{
			for (int i = 0; i < board.boardWords.Count; i++)
			{
				RoyalWord_GridBoardWorker.RoyalWord_BoardWord boardWord = board.boardWords[i];
