...
namespace Game
{
	public class WordNexus_RecyclableListHandler<T>
	{
		#region Classes
		...
		private WordNexus_RecyclableListItem<T> listItemPrefab;
		...
		public System.Action<WordNexus_RecyclableListItem<T>> OnListItemCreated { get; set; }
		...
		public WordNexus_RecyclableListHandler(List<T> dataObjects, WordNexus_RecyclableListItem<T> listItemPrefab, RectTransform listContainer, ScrollRect listScrollRect)
		{
		...
		public void Refresh()
		{
			for (int i = topItemIndex; i <= bottomItemIndex; i++)
			{
				...
				{
					WordNexus_RecyclableListItem<T> listItem = placeholder.GetChild(0).GetComponent<WordNexus_RecyclableListItem<T>>();
		...
		private void AddListItem(int index, RectTransform placeholder, bool addingOnTop)
		{
			...
			WordNexus_RecyclableListItem<T> listItem = listItemPool.GetObject<WordNexus_RecyclableListItem<T>>(placeholder, out itemInstantiated);
