...
namespace Game
{
	public class RoyalWord_SaveManager : SingletonComponent<RoyalWord_SaveManager>
	{
		#region Member Variables

		private List<RoyalWord_ISaveable> saveables;
		...
		private List<RoyalWord_ISaveable> Saveables
		{
			get
			{
				if (saveables == null)
				{
					saveables = new List<RoyalWord_ISaveable>();
				}
		...
		{
			if (!System.IO.File.Exists(RoyalWord_SaveManager.Instance.SaveFilePath))
			{
			...
			bool delete = UnityEditor.EditorUtility.DisplayDialog("Delete Save File", "Delete the save file located at " + RoyalWord_SaveManager.Instance.SaveFilePath, "Yes", "No");

			if (delete)
			{
				System.IO.File.Delete(RoyalWord_SaveManager.Instance.SaveFilePath);
		...
		public static void PrintSaveFileToConsole()
		{
			...
			string contents = System.IO.File.ReadAllText(RoyalWord_SaveManager.Instance.SaveFilePath);
		...
		/// </summary>
		public void Register(RoyalWord_ISaveable saveable)
		{
		...
		/// </summary>
		public JSONNode LoadSave(RoyalWord_ISaveable saveable)
		{
